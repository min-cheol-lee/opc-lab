...
- Optional dev fallback keys for internal testing:
  - `litopc_user_id_v1` (localStorage)
  - `litopc_email_v1` (localStorage)

## Backend (Render/Railway)
- `LITOPC_DB_PATH` (if sqlite file path override needed)
- `LITOPC_ADMIN_TOKEN` (required for admin endpoints)
...
- `BILLING_MODE=stub`
- Use `x-litopc-user-id` + `x-litopc-email` fallback from frontend localStorage.
- Tester login UI:
